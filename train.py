...
if __name__ == "__main__":
    ...
    if os.path.isfile(train_saved_path):
        ...
        print(
            f"Load: {i}; test_loss: {np.round(best_test_loss_mean, 5)}; train_loss: {np.round(best_train_loss_mean, 5)}")
    else:
        ...
        for batch in train_loader:
            ...
            if i != 0 and i % hp.step_print == 0:
                ...
                logger.add_scalar("Loss/train_loss",
                                  train_loss_mean, global_step=i)

                if i % hp.step_test == 0:
                    test_loss_mean = calculate_test_loss(model, test_loader)
                    audio, fig = inference_utterance(model, "Hello, World.")

                    logger.add_scalar("Loss/test_loss",
                                      test_loss_mean, global_step=i)
                    logger.add_figure(f"Img/img_{i}", fig, global_step=i)
                    logger.add_audio(
                        f"Utterance/audio_{i}", audio, sample_rate=hp.sr, global_step=i)
                ...
                print(
                    f"{epoch}-{i}) Train loss: {np.round(train_loss_mean, 5)}; Duration: {time_sec} sec.")
                train_loss_mean = 0.0
